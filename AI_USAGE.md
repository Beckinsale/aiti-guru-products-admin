...
## Сессия 6 — Очистка кода

**Промпт:** Удали мусор: неиспользуемые импорты, переменные, компоненты, закомментированный код. Удали отладку: console.log, debugger, временные комментарии. Оставь только осмысленные JSDoc/TS-комментарии. Убедись что код проходит ESLint и TypeScript strict.

**Результат:**
- Исправлены все ESLint ошибки:
  - Убран экспорт `buttonVariants` (fast refresh issue в `button.tsx`)
  - Удален пустой интерфейс `InputProps` (заменен на inline тип)
  - Перемещён `AuthContext` в отдельный файл (`src/features/auth/context/AuthContext.ts`)
  - Завёрнуты `login` и `logout` в `useCallback`
  - Удалена неиспользуемая переменная `_values` в `AddProductModal`
- 0 ошибок TypeScript (`tsc --noEmit` ✓)
- Проверено: нет `console.log`, `debugger`, закомментированного кода
- Все комментарии осмысленные (@req требования + non-obvious логика)
- 23/23 E2E тесты проходят ✓
- Build успешен ✓
...
- Рефакторинг кода (структурные улучшения, перемещение файлов)
```
